...
import type { AdminAuditLog, AdminDepartment, AdminNotification, AdminRole, AdminState, AdminUser } from "./types";
...
let state: AdminState = { users: initialUsers, departments: initialDepartments, categories: categories.map((item) => ({ ...item })), roles: initialRoles, notifications: initialNotifications, auditLogs: initialAuditLogs, settings: structuredClone(mockSettings) };
...
export const adminStore = {
  ...
  categories: {
    create: (value: Omit<DocumentCategory, "id">) => update((s) => ({ ...s, categories: [{ ...value, id: id("category") }, ...s.categories] })),
    update: (categoryId: string, value: Partial<DocumentCategory>) => update((s) => ({ ...s, categories: s.categories.map((item) => item.id === categoryId ? { ...item, ...value } : item) })),
